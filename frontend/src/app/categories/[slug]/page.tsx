...
export default async function CategoryPage({
  ...
  if (referenceProducts.length) {
    ...
    return (
      /*
       * ROOT FIX: Remove overflow-hidden from <section>.
       * overflow-hidden was the direct cause of the pill strip being clipped.
       * Each child that needs clipping (hero, cards) has its own overflow-hidden.
       */
      <section className="w-full min-w-0 space-y-6 px-4 sm:px-6 lg:px-0">
        ...
        {/* Hero Header — has its own overflow-hidden for decorative circles */}
        ...
        {/*
         * MOBILE PILL STRIP — placed OUTSIDE the grid, at section level.
         * -mx-4 sm:-mx-6 cancels section padding so the strip reaches screen edges,
         * px-4/sm:px-6 on the inner div re-adds padding for the first pill.
         * overflow-x-auto works correctly here because no ancestor clips it.
         * Hidden on lg+ where the sidebar takes over.
         */}
        {availableTags.length ? (
          <div className="lg:hidden -mx-4 sm:-mx-6">
            <div
              className="flex items-center gap-2 overflow-x-auto px-4 pb-2 sm:px-6"
            ...
            >
              ...
              >
                All ({referenceProducts.length})
              </Link>
              {availableTags.map((tagItem) => {
                const isActive = selectedTag?.toLowerCase() === tagItem.name.toLowerCase();
                return (
                  <Link
                    key={tagItem.name}
        ...
        <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
          ...
          {availableTags.length ? (
            <aside className="hidden lg:block lg:h-fit lg:rounded-2xl lg:border lg:border-slate-200 lg:bg-white lg:shadow-sm lg:sticky lg:top-6">
              ...
              <div className="p-2">
                ...
                >
                  <span>All</span>
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${!selectedTag ? "bg-white/20 text-white" : "bg-slate-100 text-slate-500"}`}>
                  ...
                  {availableTags.map((tagItem) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${isActive ? "bg-white/20 text-white" : "bg-slate-100 text-slate-500"}`}>
                          {tagItem.count}
                        </span>
                      </Link>
                    );
                ...
                <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                  {paginatedProducts.map((product) => (
                    ...
                    >
                      ...
                      {/* min-w-0 + line-clamp + break-words prevent text overflow */}
                        ...
                        {product.filterTags?.length ? (
                          <div className="mt-3 flex flex-wrap gap-1.5">
                            {product.filterTags.slice(0, 3).map((t) => (
                              <span key={t} className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                                {t}
                              </span>
                            ))}
                          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((item, idx) => item === "…" ? (
                ...
                <Link key={item} href={buildHref({ page: item as number })} className={`flex h-9 w-9 items-center justify-center rounded-lg text-sm font-medium transition active:scale-95 ${safePage === item ? "bg-slate-900 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-600 hover:border-blue-300 hover:text-blue-700"}`}>{item}</Link>
              ))}
            {safePage < totalPages ? (
